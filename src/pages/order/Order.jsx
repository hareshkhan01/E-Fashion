...
function Order() {
  const userid = JSON.parse(localStorage.getItem('user_login')).user.uid
                  ...
                  <div className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
                    ...
                    <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                      <div className="mt-5 sm:mt-0">
                        ...
                        <div className='m-2 p-2'><h2>Pending</h2></div>
                      </div>
                    </div>
                    <div>
                      {order._status !== "Cancelled" ? (
                        <button className="rounded-full p-2 bg-red-800 text-white" onClick={cancelProduct}>Cancel</button>
                      ) : null}
                    </div>
                  </div>
                </div>
