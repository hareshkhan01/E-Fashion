...
function ProductCard() {
    ...
    // Filter and sort products by time in descending order
    const latestProducts = product
        .filter((obj) => obj.title.toLowerCase().includes(searchkey.toLowerCase()))
        .filter((obj) => obj.category.toLowerCase().includes(filterType.toLowerCase()))
        .filter((obj) => obj.price.includes(filterPrice))
        .filter((obj) => parseInt(obj.number_of_product, 10) > 0)
            ...
            <div className="container px-5 py-8 md:py-16 mx-auto">
                ...
                <div className="flex flex-wrap -m-4">
                    {latestProducts.map((item, index) => {
                        const { title, price, imageUrl, number_of_product } = item;
                        return (
                                ...
                                <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '' }}>
                                    ...
                                    <div className="p-5 border-t-2">
                                        ...
                                        <div className="flex justify-center">
                                            <button onClick={() => { user ? addCart(item) : window.location.href = "/login" }} type="button" className="focus:outline-none text-white bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full py-2">Add To Cart</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
